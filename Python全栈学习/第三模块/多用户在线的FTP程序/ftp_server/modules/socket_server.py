...
#  本来计划使用socketserver模块实现多用户并发登陆，时间有限，暂未深入研究
...
class MyTCPHandler(socketserver.BaseRequestHandler):
    def handle(self):
        print("客户端IP： %s ，端口： %s  连接服务器！" %(self.client_address[0], self.client_address[1] ))
        while True:
            try:

                # self.data = self.request.recv(1024).strip()
                recv_data = pickle.loads(self.request.recv(8192))

                if not recv_data:
                    print("Error: %s 客户端IP【%s】端口【%s】 断开连接！ " %(self.client_address[0], self.client_address[1]))
                    break
                print(" 【%s】端口写着: %s" % (self.client_address[1], recv_data))
                ...
                if hasattr(cli, action):
                    ...
                    self.request.send(pickle.dumps("Error!"))
    ...
    def login(self, request):
        ...
        print(send_data)
        # self.request.send(pickle.dumps(send_data))
    ...
    def put(self, request):
        ...
        if recv_data['status']:
            ...
            if server_file_md5 == recv_data['file_md5']:
                ...
                self.request.send(pickle.dumps(send_msg))
            else:
                if server_file_md5:
                    filesize = os.path.getsize(abs_file_path)
                else:
                    filesize = 0
                space_aviable = pickle.loads(self.df(""))['aviable_space'] * 1024 * 1024 + filesize - recv_data[
                    'filesize']
                send_msg = {"filename": filename,
                            "server_file_md5": server_file_md5,
                            "filesize": filesize,
    ...
    def get(self, request):
        ...
        if os.path.isfile(abs_file):
            ...
            # 接收客户端开始传输的指令
            res2 = pickle.loads(self.request.recv(8192))
            print(res2)
