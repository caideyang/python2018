...
class FTPServer(object):
    ...
    def send_data(self, **kwargs):
        ...
        self.conn.send(pickle_data)

    def recv_data(self):
        data = self.conn.recv(settings.MAX_RECV_BYTE)
        if not data:
            return False
        return pickle.loads(data)

    def recv_byte_data(self, to_trans_size, filename=None, mode=None, file_md5=None, type='msg'):
    ...
    def is_authorized(self, request_data):
    ...
    def _df(self, request_data):
        if self.is_authorized(request_data):
            request_data['status'] = 200
            request_data['space_info'] = self.login_user_list[request_data['username']].space_info
            self.send_data(**request_data)

    def _ls(self, request_data):
        if self.is_authorized(request_data):
            ...
            if os.path.isdir(abs_path):
                ...
                self.send_data(**request_data)  # 将数据的长度信息发送给客户端
                self.conn.send(msg.encode(settings.DEFAULT_BYTE_CODE))

            else:
                request_data['status'] = 403
                self.send_data(**request_data)

    def _mkdir (self, request_data):
        if self.is_authorized(request_data):
            if len(request_data["args"]) != 2:
    ...
    def _put(self, request_data):
        ...
        if self.is_authorized(request_data):
            ...
            request_data["status"] = self.recv_byte_data(request_data["filesize"] - request_data["seek_place"],
